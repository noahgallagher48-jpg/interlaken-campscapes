...
## Log
- 2026-09-17: prints.html, the framed limited editions section now plays the bridge film (film/bridge_on_the_wall.mp4, 720x1280 web copy of the accepted 1080 film, poster beside it). The PDF is unchanged; the film lives on the page only.
- 2026-08-22 (the web tier becomes real web, + Ramah-grade page): every frame now carries its print sizes, computed from the MASTER's true resolution (dims read off the Drive files by ranged header fetches, _work/master_dims.json); frames whose shape has no standard size say custom cut. The field guide joined the verbs row. WEB REBUILT AT THE STANDARD: 3840px q90 4:4:4 from the masters, credit-stacked, capture dates restored from _work/times.json (the July exports never carried them); 198 files at HiResNG_clean/web on Drive (public by inheritance), per-frame Web buttons are one-click Drive downloads, ids in _work/drive_web_ids.json. All-for-web zip = GitHub release asset (552MB, interlaken-2026-web), honest size on the button; selection Web fires per-frame Drive downloads like Full res. The 2560 present tier is display furniture again, nowhere behind a download button.
- 2026-08-17 (the call, and the question): follow-up meeting held, 10:00am. From it: the survey layout is approved and the vote stays open through Labor Day, Sept 7. One question added to the send sheet at the client's request, verbatim from the call: "What does your favorite image bring back for you?" with "A moment, a sound, a summer." as the placeholder; optional, submits as what_it_brings_back alongside the five pick lists. Answers arrive in the same vote email.
- 2026-08-16 (ballot swept before it travels): Drea confirmed Monday Aug 17 10:00am and that the favorites link has not gone out yet; she has her distribution list ready and asked to keep the vote open. Zero real submissions to date (only the owner's Aug 4 test), so the page could be rebuilt safely. Swept the hardcoded ballot lists against the kill list (13 and 108 replaced by their keepers 12 and 109, 194 out; 89 frames, still 25 picks) and fixed a real submission bug: the Shabbat section's seven picks were collected but never included in the Web3Forms payload, so every vote would have arrived missing them. localStorage key bumped so stale test picks cannot ride along. build_vote.py now refuses to build if a killed frame appears in its hardcoded lists, closing the one gap the kill-list filter did not cover.
...
- 2026-09-15: prints.html investment tables fixed (label column was collapsing), type pass, prints.pdf regenerated, pushed. agreement.html drafted locally (the living agreement page, version 1, seven questions for the camp, Web3Forms answers), NOT committed or pushed; waits for Noah on the two open decisions and the addressee.
